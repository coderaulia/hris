...
import { loginAs, openSidebarLink, expectSuccessToast, uniqueLabel, currentPeriod } from './support/app.js';
import { signInAs, restRequest, encodeFilterValue } from './support/supabase-api.js';

test('manager KPI definition moves from submitted to approved through governance', async ({ browser }) => {
    ...
    const approvalToggle = superadminPage.locator('#kpi-approval-required-toggle');
    if (!(await approvalToggle.isChecked())) {
        await approvalToggle.check();
        await superadminPage.getByRole('button', { name: /Save Governance Rule/i }).click();
    ...
    await managerPage.locator('#kpi-def-desc').fill('Playwright regression KPI approval flow');
    await managerPage.locator('#kpi-def-category').selectOption({ label: 'Frontend Engineer' });
    ...
    await managerPage.locator('#kpi-def-request-note').fill('Regression coverage for manager approval queue.');
    await managerPage.getByRole('button', { name: /^Save$/i }).click();

    await expectSuccessToast(managerPage, /submitted for HR approval/i);
    await expect(managerPage.locator('#kpi-pending-approvals')).toContainText(kpiName);
    ...
    const pendingCard = superadminPage.locator('#kpi-pending-approvals .border.rounded').filter({ hasText: kpiName }).first();
