...
async function selectScopedEmployee(page, employeeName) {
	const employeeId = await page
		.locator("#inp-pending-select")
		.evaluate((select, name) => {
			const option =
				[...select.options].find((item) => item.text.includes(name)) ||
				select.options[1];
			if (!option) return "";
	...
	if (!employeeId) {
		throw new Error(
			`Unable to find scoped employee "${employeeName}" in assessment queue.`,
		);
	}
...
test("manager can submit an assessment and RLS blocks another manager from editing it", async ({
	page,
}) => {
	...
	// Wait for modal to fully load and become visible
	await page.waitForSelector("#step-form", {
		state: "visible",
		timeout: 10000,
	});
	await expect(page.locator("#step-form")).toBeVisible();
	await expect(
		page.locator("#questions-area .form-range").first(),
	).toBeVisible();
	...
	assertDenied(
		deniedUpdate,
		"other manager cannot patch another manager assessment",
	);
});
