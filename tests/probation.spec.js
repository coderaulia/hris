...
test('probation and PIP flow covers create, extend, and close states', async ({ page }) => {
    await loginAs(page, 'manager');
    await openSidebarLink(page, 'Records', 'Probation & PIP');

    await page.locator('#probation-period-filter').fill('2026-03');

    await page.getByRole('button', { name: /Generate Probation Drafts/i }).click();
    ...
    await expect(page.locator('.swal2-popup')).toContainText('Probation Review Form');
    ...
    await page.locator('#pr-summary').fill('Probation extended for another review cycle during regression test.');
    await page.locator('.swal2-confirm').click();

    await expectSuccessToast(page, /Probation review updated/i);
    await expect(page.locator('#probation-reviews-body')).toContainText(/extend/i);
    ...
    await expect(page.locator('.swal2-popup')).toContainText('Update PIP Status');
    await page.locator('.swal2-popup select').selectOption('completed');
    await page.locator('.swal2-confirm').click();

    await expect(page.locator('.swal2-popup')).toContainText('PIP Summary / Update Note');
    await page.locator('.swal2-popup textarea').fill('Closed by Playwright regression coverage.');
    await page.locator('.swal2-confirm').click();
